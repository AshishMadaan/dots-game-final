...
const getAnimationTime = (fallRate) => {
  ...
  return gameBoard.clientHeight / fallRate;
};
...
const updateDotsSpeed = (value) => {
  const gameBoard = document.querySelector(".game__board");
  const dots = gameBoard.querySelectorAll(".dot");
  dots.forEach((dot) => {
    dot.style.animation = `slideDown ${getAnimationTime(value)}s linear`;
  });
};
